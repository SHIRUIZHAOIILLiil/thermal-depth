...
class MarigoldTrainer:
    def __init__(
        ...
        # self.model.encode_text_to_rgb.requires_grad_(True)
        ...
        # print(self.prediction_type,flush=True) v_prediction
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                with torch.no_grad():
                    ...
                    # print(gt_depth_latent.shape, flush=True)
                    # Encode image
                    rgb_latent = self.model.encode_rgb(rgb)  # [B, 4, h, w]
                    # rgb_latent = self.model.encode_text("").unsqueeze(-1).unsqueeze(-1)
                    # rgb_latent = rgb_latent.repeat(gt_depth_latent.shape[0],1,gt_depth_latent.shape[2],gt_depth_latent.shape[3])
                    # rgb_latent += self.model.positional_encoding_2d(rgb_latent)
                    # rgb_latent = self.model.encode_text_to_rgb(rgb_latent.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)

                # Sample a random timestep for each image
                # print(self.scheduler_timesteps,flush=True) # 1000
                timesteps = torch.randint(
                    # self.scheduler_timesteps - 1, # one-step-training, default: 0
                    0,
                    self.scheduler_timesteps,
                    (batch_size,),
                    device=device,
                ...
                text_embed = []
                text_embed_global = []
                for i, rgb_path in enumerate(batch["rgb_relative_path"]):
                    if rgb_path[0:2]=="ai":
                        # Hypersim
                        txt_file = "./Marigold_text/hypersim/train/" + rgb_path[:-4] + ".txt"
                    else:
                        txt_file = "./Marigold_text/vkitti/train/" + rgb_path[:-4] + ".txt"
                    # print(txt_file,flush=True)

                    with open(txt_file, 'r') as file:
                        # Read the first line
                        lines = file.readlines()
                        random_line = random.choice(lines)
                        input_text = random_line.strip()
                        # input_text = ""
                    # print(input_text,flush=True)
                    # if timesteps[i]> 1000: # > prior threshold
                    #     pass # prior, using text
                    # else:
                    #     input_text = "" # postrior, using image
                    # input_text="" # remove
                    # Text embedding
                    text_embed_temp = self.model.encode_text(input_text).to(device)
                    text_embed_global.append(torch.mean(text_embed_temp,dim=1))
                    text_embed_global.append(torch.mean(text_embed_temp,dim=1))
                    text_embed_temp = F.pad(text_embed_temp, (0, 0, 0, 77 - text_embed_temp.shape[1]), mode='constant', value=0)
                    text_embed.append(text_embed_temp)
                    # print(text_embed_temp.shape,flush=True)

                text_embed = torch.cat(text_embed, dim=0)
                text_embed_global = torch.cat(text_embed_global, dim=0)
                # print(text_embed.shape,flush=True)
                ...
                # Apply the padding
                # [B, 2, 1024] # might need change to 77?
                # print(text_embed,flush=True)
                ...
                #     if timesteps[i] > 1000: # prior threshold
                #         rgb_latent[i] = torch.rand_like(rgb_latent[i]) # prior, using rand. Otherwise, using both
                ...
                if "sample" == self.prediction_type:
                    ...
                    target = self.training_noise_scheduler.get_velocity(
                        gt_depth_latent, noise, timesteps
                    )  # [B, 4, h, w] # what the depth_latent is after adding timesteps times of noise
                # The final velocity is calculated as the weighted combination of the scaled noise
                # and the scaled sample. This gives a measure of how fast the noise is being removed
                # and how the image is changing with respect to time.
                else:
                ...
                # loss_kld = (0.5 * (mean ** 2 + std ** 2 - torch.log(std ** 2) - 1)).sum(dim=1).mean()
                # loss += 1e-3 * loss_kld
    ...
    def validate_single_dataset(
        ...
        for i, batch in enumerate(
            ...
            # rgb_int = torch.zeros_like(rgb_int)
            # GT depth
            ...
            # print(batch["rgb_relative_path"], flush=True)
            # input_text = "Something meaningful."
            for rgb_path in batch["rgb_relative_path"]:
                txt_file = "./Marigold_eval_text/" + rgb_path[:-4] + ".txt"
                with open(txt_file, 'r') as file:
                    # Read the first line
