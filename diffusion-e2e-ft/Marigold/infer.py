...
if "__main__" == __name__:
    ...
    # add: Automatically find corresponding JSON file based on dataset name
    dataset_name = cfg_data.name
    # Map dataset names to JSON file names (handle special cases)
    dataset_name_to_json = {
        "nyu_v2": "nyu",
        "kitti": "kitti",
        "eth3d": "eth3d",
        "diode": "diode",
    ...
    json_name = dataset_name_to_json.get(dataset_name, dataset_name)
    text_descriptions_json = f"internVL/outputs/eval/{json_name}_depth_descriptions.json"
    
    # Strictly require JSON file to exist for evaluation
    if not os.path.exists(text_descriptions_json):
        raise FileNotFoundError(
            f"Text descriptions JSON file not found: {text_descriptions_json}. "
            f"Every image must have a text description for evaluation. "
            f"Please ensure the JSON file exists for dataset '{dataset_name}'."
        )
    ...
    dataset: BaseDepthDataset = get_dataset(
        ...
        text_descriptions_json=text_descriptions_json,  # add: pass text descriptions JSON
    )
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            # add: Get text description from batch - strictly require non-empty text
            # batch["text"] is a list with one element (batch_size=1)
            if "text" not in batch:
                raise ValueError(f"Batch is missing 'text' key. Every image must have a text description for evaluation.")
            
            text_prompt = batch["text"][0]
            
            # Strictly require non-empty text description
            if text_prompt is None:
                raise ValueError(f"Image has None text description. Every image must have a non-empty text description for evaluation.")
            if not isinstance(text_prompt, str):
                raise ValueError(f"Image has non-string text description (type: {type(text_prompt)}). Every image must have a string text description.")
            if len(text_prompt.strip()) == 0:
                raise ValueError(f"Image has empty text description. Every image must have a non-empty text description for evaluation.")

            # add
            if model_type == "marigold":
                pipe_out = pipe(
                    input_image,
                    ...
                    normals = False,
                    noise   = noise,
                    prompt  = text_prompt,  # add: pass text prompt
                )
            elif model_type == "geowizard":
