...
def main():
    ...
    # Training datasets
    hypersim_root_dir = "../Marigold/data/hypersim/"
    vkitti_root_dir   = "../Marigold/data/vkitti/"   
    hypersim_split_csv = os.path.join(hypersim_root_dir, "train", "filename_meta_train.csv")
    
    # Text description JSON files
    hypersim_text_json = "path/to/hypersim_rgb_descriptions.json"
    vkitti_text_json = "path/to/vkitti_depth_descriptions.json"
    
    train_dataset_hypersim = Hypersim(root_dir=hypersim_root_dir, split_csv_path=hypersim_split_csv, transform=True, text_descriptions_json=hypersim_text_json)
    train_dataset_vkitti   = VirtualKITTI2(root_dir=vkitti_root_dir, transform=True, text_descriptions_json=vkitti_text_json)
    ...
    global_step = 0
    first_epoch = 0
    saved_first_batch = False  # Flag to track if first batch has been saved
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):     
                ...
                # Generate UNet prediction with text encoding
                # Get text descriptions from batch
                # -----Strictly require that every image has non-empty text
                if "text" not in batch:
                    raise ValueError(f"Batch is missing 'text' key. Every image must have a text description.")
                
                text_descriptions = batch["text"]
                if text_descriptions is None:
                    raise ValueError(f"Batch 'text' is None. Every image must have a text description.")
                
                # Ensure text_descriptions is a list
                if isinstance(text_descriptions, str):
                    text_descriptions = [text_descriptions]
                
                # Check that we have the same number of texts as images
                num_images = len(batch["rgb"])
                if len(text_descriptions) != num_images:
                    raise ValueError(f"Mismatch: batch has {num_images} images but {len(text_descriptions)} text descriptions. Every image must have exactly one text description.")
                
                # Check that every text description is non-empty
                #commented for the sake of time
                # for idx, text in enumerate(text_descriptions):
                #     if text is None:
                #         raise ValueError(f"Image {idx} in batch has None text description. Every image must have a non-empty text description.")
                #     if not isinstance(text, str):
                #         raise ValueError(f"Image {idx} in batch has non-string text description (type: {type(text)}). Every image must have a string text description.")
                #     if len(text.strip()) == 0:
                #         raise ValueError(f"Image {idx} in batch has empty text description. Every image must have a non-empty text description.")
                
                # -------End of strictly require that every image has non-empty text
                
                # Tokenize and encode text descriptions
                text_tokens = tokenizer(text_descriptions, padding="max_length", truncation=True, return_tensors="pt", max_length=77).input_ids
                text_tokens = text_tokens.to(accelerator.device)
                encoder_hidden_states = text_encoder(text_tokens, return_dict=False)[0]
                
                # -------Save first batch images and texts for checking
                if not saved_first_batch and accelerator.is_main_process:
                    check_dir = os.path.join(args.output_dir, "batch_checks")
                    os.makedirs(check_dir, exist_ok=True)
                    
                    # Convert RGB tensor from [-1, 1] to [0, 1] for saving
                    rgb_images = batch["rgb"].cpu()
                    rgb_images = (rgb_images + 1.0) / 2.0  # [-1, 1] -> [0, 1]
                    rgb_images = torch.clamp(rgb_images, 0, 1)
                    
                    # Save images
                    save_path = os.path.join(check_dir, "first_batch_images.png")
                    save_image(rgb_images, save_path, nrow=min(4, len(rgb_images)))
                    
                    # Save text descriptions
                    text_path = os.path.join(check_dir, "first_batch_texts.txt")
                    with open(text_path, 'w', encoding='utf-8') as f:
                        f.write(f"First Batch - Batch Size: {len(text_descriptions)}\n")
                        f.write("=" * 80 + "\n\n")
                    ...
                    logger.info(f"Saved first batch check: images to {save_path}, texts to {text_path}")
                    saved_first_batch = True
                
                # -------End of saving first batch images and texts for checking
                unet_input = (
                ...
                if val_mask.any():
                    ...
                    #mod
                    current_latent_estimate = current_latent_estimate.to(dtype=vae_dtype(vae))
                    #---
