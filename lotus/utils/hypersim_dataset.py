...
class HypersimImageDepthNormalTransform:
    ...
    def to_tensor_and_resize_normal(self, normal):
        ...
        normal = torch.from_numpy(normal).permute(2,0,1).unsqueeze(0)
        # resize
        normal = F.interpolate(normal.float(), size=self.size, mode='nearest').squeeze(0)
        # shape = (3, H, W)
        #mod fp16
        return normal
...
def get_hypersim_dataset_depth_normal(data_dir, resolution, random_flip, norm_type, truncnorm_min, align_cam_normal=False, split='train', text_descriptions_path=None):
    ...
    if text_descriptions_path and os.path.exists(text_descriptions_path):
        ...
        print(f"Loaded {len(text_descriptions)} text descriptions from {text_descriptions_path}")
    ...
    for root, dirs, files in os.walk(split_dir):
        for file in files:
            if file.endswith("tonemap.jpg"): 
                ...
                # mod Get text description according to image path
                text_desc_data = text_descriptions.get(image_path, {})
                ...
                data_dict["text_description"].append(text_desc)
    ...
    def preprocess_hypersim(examples):
        ...
        # mod Add text descriptions
        examples["text_descriptions"] = examples[text_column]
    ...
    def collate_fn_hypersim(examples):
        ...
        text_descriptions = [example["text_descriptions"] for example in examples]
