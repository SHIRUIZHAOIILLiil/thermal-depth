...
class MarigoldPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        noise="gaussian",
        normals=False,
        prompt: Union[str, None] = None,  # add: text prompt for conditioning
    ) -> MarigoldDepthOutput:
        """
        ...
        for batch in iterable:
            ...
            pred_raw = self.single_infer(
                ...
                noise=noise,
                normals=normals,
                prompt=prompt,  # add: pass text prompt
            )
    ...
    def encode_empty_text(self):
        ...
        text_inputs = self.tokenizer(
            prompt,
            padding="max_length",  # add: use max_length padding to match text embedding dimension
    ...
    def single_infer(
        ...
        if prompt is not None and len(prompt.strip()) > 0:
            ...
            batch_text_embed = text_embed.repeat((rgb_latent.shape[0], 1, 1))  # [B, 77, 1024]
        else:
            # Use empty text embedding
