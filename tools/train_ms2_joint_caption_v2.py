"""Jointly train Adapter V2.3 and the Lotus U-Net with RGB-derived captions.

Phase H caption run.  Identical protocol to the no-caption joint run
(``train_ms2_joint_adapter_unet_v2.py``) except that each sample's manifest
caption conditions both the frozen teacher U-Net and the student U-Net
through cross-attention.  A frozen, deterministic per-sample dropout replaces
the caption with the empty string so that empty-caption inference stays
in-distribution.  No depth GT, Val, Test, V1 checkpoint, or Thermal-VAE
inference path is used.
"""
...
CHECKPOINT_FORMAT = "adapter_unet_joint_caption_v2_full_epoch_response"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/lotus_line_v2/full_train_epoch1_joint_caption_v2"),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Resume only this exact joint caption run after interruption.",
    )
    parser.add_argument(
        "--smoke-updates",
        type=int,
...
def caption_dropped(seed: int, manifest_index: int, probability: float) -> bool:
    """Frozen deterministic per-sample caption dropout decision."""
...
def validate_protocol(args) -> None:
    if args.micro_batch_size != 1 or args.gradient_accumulation_steps != 4:
        raise ValueError("Joint caption V2 requires micro-batch 1 and accumulation 4.")
    if args.timestep != 999:
        raise ValueError("Joint caption V2 must match inference timestep 999.")
    ...
    if min(
        ...
        args.unet_max_grad_norm,
        args.condition_weight,
        args.response_weight,
        args.cosine_loss_weight,
        args.channel_stats_loss_weight,
        args.spatial_gradient_loss_weight,
        args.multiscale_gradient_loss_weight,
        ...
        raise ValueError("Learning rates, clipping, and loss weights must be non-negative.")
...
def validate_trainability(adapter, student_unet, frozen_modules) -> None:
    if not any(parameter.requires_grad for parameter in adapter.parameters()):
        raise RuntimeError("Joint Adapter has no trainable parameters.")
    if not any(parameter.requires_grad for parameter in student_unet.parameters()):
        raise RuntimeError("Joint student U-Net has no trainable parameters.")
    for module in frozen_modules:
        if any(parameter.requires_grad for parameter in module.parameters()):
            raise RuntimeError("A frozen joint support module is trainable.")
...
def build_sample(row, anythermal, lotus, teacher_unet, prompt, args, device, teacher_dtype):
    """Prepare features, thermal, teacher condition/response, latent input for one sample."""
...
def forward_losses(adapter, student_unet, prompt_fp32, sample, args, device):
    features, thermal_tensor, teacher_condition, latent_input, teacher_response = sample
...
def save_checkpoint(path, adapter, student_unet, optimizer, step, next_offset, permutation, args, manifest_hash):
    payload = {
        ...
        "caption_conditioned_training": True,
...
def main() -> None:
    ...
    dropout_count = sum(
        1 for row in rows if caption_dropped(args.seed, row["manifest_index"], args.caption_dropout)
    )
    ...
    frozen_config = {
        "route": "pure AnyThermal + Adapter V2.3 + jointly trained Lotus U-Net + RGB-derived caption",
        "training_objective": (
            "V2.4 condition distillation + frozen-teacher response/gradient consistency, "
            "joint Adapter+U-Net updates, caption-conditioned teacher and student"
        ),
        "initialization": (
            "fresh random Adapter V2.3; pretrained Lotus U-Net (fp16 load, fp32 student cast); "
            "fresh optimizer"
        ),
        "teacher_training_only": "frozen Thermal-VAE mode condition + frozen pretrained U-Net copy",
        "inference_uses_thermal_vae": False,
        "caption_source": "manifest captions (InternVL3-8B on paired RGB)",
        "caption_dropout_rule": "random.Random(f'{seed}:caption_dropout:{manifest_index}').random() < p",
        "caption_dropout_probability": args.caption_dropout,
        "caption_dropout_samples": dropout_count,
        "caption_truncation": "CLIP tokenizer 77-token limit; longer captions truncated silently",
        ...
        **shape,
        ...
        "no_caption_joint_checkpoint_used": False,
    ...
    validate_trainability(
        adapter, student_unet, (lotus.vae, lotus.text_encoder, teacher_unet, anythermal.model)
    )
    ...
    if args.resume is not None:
        ...
        if checkpoint.get("format") != CHECKPOINT_FORMAT:
            raise RuntimeError("Resume checkpoint is not a formal joint caption run.")
        ...
        if checkpoint.get("settings") != serializable_settings(args):
            raise RuntimeError("Resume settings differ from the frozen joint caption run.")
    ...
    if start_step == 0 and next_offset == 0 and args.smoke_updates is None:
        save_checkpoint(
            output / "joint_caption_step_0000.pt",
            adapter,
            student_unet,
            optimizer,
            0,
    ...
    with log_path.open("a", encoding="utf-8") as log_handle:
        for step in range(start_step + 1, total_updates + 1):
            ...
            for row_index in batch_indices:
                ...
                features, thermal_tensor, teacher_condition, latent_input, teacher_response, diagnostics = build_sample(
                    row, anythermal, lotus, teacher_unet, prompt, args, device, teacher_dtype
                )
                if len(first_diagnostics) < 8:
                    diagnostics["caption_dropped"] = dropped
                ...
                losses = forward_losses(
                    ...
                    (features, thermal_tensor, teacher_condition, latent_input, teacher_response),
                    args,
                    device,
                )
            ...
            if any(parameter.grad is not None for parameter in lotus.text_encoder.parameters()):
                raise RuntimeError("Frozen text encoder unexpectedly owns gradients.")
            adapter_grad_norm = torch.nn.utils.clip_grad_norm_(
                adapter.parameters(), float("inf")
            )
            ...
            if args.smoke_updates is None and (
                ...
                name = (
                    "joint_caption_end.pt"
                    if step == total_updates
                    else f"joint_caption_step_{step:04d}.pt"
                )
                save_checkpoint(
                    output / name,
                    adapter,
    ...
    summary = {
        **frozen_config,
        ...
        "captions_used": caption_used_count,
        "captions_dropped": caption_dropped_count,
        "end_checkpoint": str(output / "joint_caption_end.pt"),
        "end_checkpoint_sha256": sha256(output / "joint_caption_end.pt"),
        "elapsed_seconds": time.time() - start_time,
        "final_training_record": last_record,
        "frozen_modules": {
