...
def run_evaluation(pipeline, task, args, step, accelerator):
    # Load text descriptions for evaluation datasets
    all_text_descriptions = {}
    
    # Map dataset names to their text description file paths
    dataset_text_paths = {
        "nyu_v2": getattr(args, 'nyuv2_text_descriptions_path', None),
        "kitti": getattr(args, 'kitti_text_descriptions_path', None),
        "scannet": getattr(args, 'scannet_text_descriptions_path', None),
        "eth3d": getattr(args, 'eth3d_text_descriptions_path', None),
        "diode": getattr(args, 'diode_text_descriptions_path', None),
    }
    
    # Fallback to general text descriptions path if provided
    if hasattr(args, 'text_descriptions_path') and args.text_descriptions_path:
    ...
    for dataset_name, text_path in dataset_text_paths.items():
        if text_path and os.path.exists(text_path):
            ...
            if text_path:
                logging.warning(f"No text descriptions found at {text_path} for {dataset_name}")
            else:
                logging.info(f"No text descriptions provided for {dataset_name}, using empty prompts")
    
    # Define prediction functions
    def gen_depth(rgb_in, pipe, prompt="", image_path=None, dataset_name=None):
        if torch.backends.mps.is_available():
                autocast_ctx = nullcontext()
        else:
        ...
        with autocast_ctx:
            ...
            if image_path and dataset_name and dataset_name in all_text_descriptions:
                ...
                if image_path in text_descriptions:
                    ...
                    logging.warning(f"⚠ No text description found for {dataset_name}/{image_path}, using empty prompt")
            elif image_path and dataset_name:
                logging.warning(f"⚠ No text descriptions loaded for dataset {dataset_name}, using empty prompt for {image_path}")
...
def parse_args():
    ...
    # args for text
    parser.add_argument(
        "--hypersim_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for hypersim dataset",
    )
    parser.add_argument(
        "--vkitti_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for vkitti dataset",
    )
    # Text descriptions for evaluation datasets
    parser.add_argument(
        "--nyuv2_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for NYUv2 evaluation dataset",
    )
    parser.add_argument(
        "--kitti_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for KITTI evaluation dataset",
    )
    parser.add_argument(
        "--scannet_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for ScanNet evaluation dataset",
    )
    parser.add_argument(
        "--eth3d_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for ETH3D evaluation dataset",
    )
    parser.add_argument(
        "--diode_text_descriptions_path",
        type=str,
        default=None,
        help="Path to JSON file containing text descriptions for DIODE evaluation dataset",
    )
    parser.add_argument(
        "--text_descriptions_path",
        type=str,
        default=None,
        help="General path to JSON file containing text descriptions for all evaluation datasets",
    )
...
def main():
    ...
    train_hypersim_dataset, preprocess_train_hypersim, collate_fn_hypersim = get_hypersim_dataset_depth_normal(
        ...
        text_descriptions_path=args.hypersim_text_descriptions_path
        )
    ...
    train_dataset_vkitti = VKITTIDataset(
        ...
        text_descriptions_path=args.vkitti_text_descriptions_path,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for _ in range(len(train_dataloader_hypersim)):
            ...
            with accelerator.accumulate(unet):
                ...
                # Get text descriptions from batch and build prompts for both halves
                ...
                # Tokenize prompts and get encoder hidden states
