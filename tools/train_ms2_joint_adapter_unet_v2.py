"""Jointly train Adapter V2.3 and the Lotus U-Net for one full epoch.

Third arm of the route ablation.  The Adapter starts from fresh random
weights and the student U-Net starts from the pretrained Lotus checkpoint;
both are updated together with per-group learning rates.  A frozen fp16 copy
of the pretrained U-Net provides teacher responses from the Thermal-VAE mode
condition, exactly as in the Adapter-only and U-Net-only arms.  No depth GT,
Val, Test, caption, V1 checkpoint, or Thermal-VAE inference path is used.
"""
...
CHECKPOINT_FORMAT = "adapter_unet_joint_v2_full_epoch_response"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/lotus_line_v2/full_train_epoch1_joint_adapter_unet_v2"),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Resume only this exact joint full run after interruption.",
    )
    parser.add_argument(
        "--smoke-updates",
        type=int,
...
def validate_protocol(args) -> None:
    if args.micro_batch_size != 1 or args.gradient_accumulation_steps != 4:
        raise ValueError("Joint V2 requires micro-batch 1 and accumulation 4.")
    if args.timestep != 999:
        raise ValueError("Joint V2 must match inference timestep 999.")
    if min(
...
def main() -> None:
    ...
    frozen_config = {
        "route": "pure AnyThermal + Adapter V2.3 + jointly trained Lotus U-Net",
        "training_objective": (
            "V2.4 condition distillation + frozen-teacher response/gradient consistency, "
            "joint Adapter+U-Net updates"
        ),
        "initialization": (
        ...
        **shape,
        ...
        "adapter_only_checkpoint_used": False,
        "unet_only_checkpoint_used": False,
    ...
    # Student starts bit-identical to the frozen teacher, mirroring the
    # U-Net-only arm's initialization.
    ...
    if args.resume is not None:
        ...
        if checkpoint.get("format") != CHECKPOINT_FORMAT:
            raise RuntimeError("Resume checkpoint is not a formal joint full run.")
        ...
        if checkpoint.get("settings") != serializable_settings(args):
            raise RuntimeError("Resume settings differ from the frozen joint run.")
    ...
    with log_path.open("a", encoding="utf-8") as log_handle:
        for step in range(start_step + 1, total_updates + 1):
            ...
            if args.smoke_updates is None and (
                ...
                name = "joint_end.pt" if step == total_updates else f"joint_step_{step:04d}.pt"
                save_checkpoint(
                    output / name,
                    adapter,
                    student_unet,
    ...
    summary = {
        **frozen_config,
        ...
        "samples_seen": next_offset,
        "end_checkpoint": str(output / "joint_end.pt"),
        "end_checkpoint_sha256": sha256(output / "joint_end.pt"),
        "elapsed_seconds": time.time() - start_time,
        "final_training_record": last_record,
        "frozen_modules": {
