...
> **天空像素的 LiDAR 覆盖率恒为零**：损失和 AbsRel 都统计不到这块 ——
> 所以从 39 m 掉到 8 m 这个变化，**在任何官方指标上都是隐形的**。
>
> ⚠️ **2026-08-05 订正**：本段原写「画面最上 1/3 覆盖率只有 1.5%」，**数字是错的**。
> 我们用的 `depth_filtered`（manifest 里 `gt_variant` 字段）在 test `16-08-46` 上：
> 全图 25.5%、上三分之一 **12.7%**、上六分之一 2.9%；1.58% 是我们**不用**的原始
> `depth` 变体的上三分之一。覆盖率要到第 50 行（画面 20% 处）才过 10%。
> 完整分带表、逐行剖面，以及「GT 掩码/损失/梯度匹配/官方评估」四处的逐行确认，
> 见 `WEEK3_LOCAL_FINDINGS_20260805.md` §1。
...
## 9.5 待议：多步去噪，一个上游自己也没用过的旋钮（2026-08-04 记录，2026-08-05 订正）

> ⚠️ **订正（2026-08-05）**：本节原标题「我们把 Lotus-G 的迭代去噪关掉了」不成立。
> 上游 `lotus/infer.py:196` 自己就写死 `num_inference_steps=1`、`timesteps=[args.timestep]`
> （`:38-41` default=999），`--mode` 取 generation 或 regression 都走这一行。
> **单步 t=999 是 Lotus 作者发布的推理配方本身**；我方三个入口是照抄，不是关掉了谁的迭代。
> 下面「单步是我们自己的实现选择」那句按此理解 —— pipeline **支持**多步，作者**没用**多步。
>
> **前置已完成**：`--num-inference-steps` / `--timestep` 已加到 thermal_vae 与 rgb 两个
> direct 工具（steps=1 完全走老路径，历史数字逐位不变），`slurm/direct.sbatch` 加了
> `STEPS` 变量。⚠️ 坑：该 checkpoint 的 DDIM 是 `leading` + `steps_offset=1`，
> 它自己的 1 步排程是 **t=1** 而非 999，两种「单步」不是一回事。
> 命令见 `WEEK3_LOCAL_FINDINGS_20260805.md` §2。

**Lotus-G 的 pipeline 支持迭代去噪，单步是实现选择，不是模型的限制。**
...
（2026-08-05：两个 direct 工具已加，`train_route_suite.py` 没加也不该加 —— 见下面「障碍」。）
...
> ⚠️ **2026-08-05 订正：这句是错的。** `lotus/train_iris_g.py:1073` 把 timestep
> `repeat(bsz)` 成一个常数（不是 `randint`），`train_scripts/train_iris_g_depth.sh:22`
> 设 `TIMESTEP=999`。**Lotus-G 发布的权重只在 t=999 上训过**，多步循环会把 t 一路降到 1，
> 对原作者权重同样是分布外使用。所以这个实验要按「排除一条借口」来读，
> 而不是按「恢复模型本来的能力」来读 —— 读法表见
> `WEEK3_LOCAL_FINDINGS_20260805.md` §2.3。
