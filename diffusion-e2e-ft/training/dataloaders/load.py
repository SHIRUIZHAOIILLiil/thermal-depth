...
class Hypersim(Dataset):
    #mod
    def __init__(self, root_dir, split_csv_path=None, transform=True, near_plane=1e-5, far_plane=65.0, text_descriptions_json=None):
        self.root_dir   = root_dir
        if split_csv_path is None:
            # CSV path based on root_dir
            split_csv_path = os.path.join(root_dir, "train", "filename_meta_train.csv")
            # If not found, try the old default path
            if not os.path.exists(split_csv_path):
                split_csv_path = os.path.join("data/hypersim/processed/train/filename_meta_train.csv")
        ...
        self.warned_paths = set()  # Track paths that have been warned about missing text
        if text_descriptions_json is not None and os.path.exists(text_descriptions_json):
            with open(text_descriptions_json, 'r') as f:
                descriptions_dict = json.load(f)
                # Convert to a simple dict mapping path -> description
                # Normalize paths for matching (use os.path.normpath to handle path variations)
                for path, data in descriptions_dict.items():
                    normalized_path = os.path.normpath(path)
                    if isinstance(data, dict) and 'description' in data:
                        self.text_descriptions[normalized_path] = data['description']
                    elif isinstance(data, str):
                        self.text_descriptions[normalized_path] = data
    ...
    def _find_pairs(self):
        ...
        for _, row in df.iterrows():
            if row['included_in_public_release'] and (row['split_partition_name'] == "train"):
                ...
                # Only require RGB and depth; normals are optional
                if os.path.exists(rgb_path) and os.path.exists(depth_path):
                    pair_info = {'rgb_path': rgb_path, 'depth_path': depth_path, 'normal_path': normal_dir if os.path.exists(normal_dir) else None}    
    ...
    def __getitem__(self, idx):
        ...
        if normal_path is not None and os.path.exists(normal_path):
            ...
            # Create a dummy normal image (gray, which represents zero normal vector)
            normal_image = Image.new('RGB', depth_image.size, (128, 128, 128))
        ...
        # Get text description if available (normalize path for matching)
        normalized_rgb_path = os.path.normpath(rgb_path)
        text_description = self.text_descriptions.get(normalized_rgb_path, "")
        
        # Check if text description is empty and warn/record if not already warned
        if not text_description and normalized_rgb_path not in self.warned_paths:
            warning_msg = f"[Hypersim] Warning: No text description found for image: {rgb_path}"
            warnings.warn(warning_msg, UserWarning)
            logging.warning(warning_msg)
            self.warned_paths.add(normalized_rgb_path)
            
        return {"rgb": rgb_tensor, "depth": depth_tensor, 'metric': metric_tensor, 'normals': normal_tensor, "val_mask": valid_depth_mask, "domain": "indoor", "text": text_description}

    
# Virtual KITTI 2.0
class VirtualKITTI2(Dataset):
    def __init__(self, root_dir, transform=None, near_plane=1e-5, far_plane=80.0, text_descriptions_json=None):
        self.root_dir = root_dir
        self.near_plane = near_plane
        self.far_plane  = far_plane
        self.pairs = self._find_pairs()
    ...
    def __getitem__(self, idx):
        ...
        # Check if text description is empty and warn/record if not already warned
        if not text_description and normalized_rgb_path not in self.warned_paths:
            warning_msg = f"[VirtualKITTI2] Warning: No text description found for image: {rgb_path}"
            warnings.warn(warning_msg, UserWarning)
            logging.warning(warning_msg)
            self.warned_paths.add(normalized_rgb_path)

        return {"rgb": rgb_tensor, "depth": depth_tensor, 'metric': metric_tensor, 'normals': normal_tensor, "val_mask": valid_depth_mask, "domain": "outdoor", "text": text_description}