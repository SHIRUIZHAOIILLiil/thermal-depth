...
class BaseDepthDataset(Dataset):
    def __init__(
        ...
        text_descriptions_json: str = None,  # add: path to JSON file with text descriptions
        **kwargs,
    ) -> None:
        super().__init__()
        self.mode = mode
        ...
        # add: Load text descriptions if provided
        ...
        if text_descriptions_json is not None and os.path.exists(text_descriptions_json):
            with open(text_descriptions_json, 'r') as f:
                ...
                # Convert to a simple dict mapping normalized path -> description
    ...
    def _get_data_item(self, index):
        ...
        # add: Get text description - strictly require non-empty text only for RGB_ONLY mode (inference)
        normalized_rgb_path = os.path.normpath(rgb_rel_path)
        text_description = self.text_descriptions.get(normalized_rgb_path, "")
        
        # Strictly require text description only for RGB_ONLY mode (inference)
        # For EVAL mode, text description is optional (not needed for evaluation)
        if self.mode == DatasetMode.RGB_ONLY:
            # Strictly require that every image has non-empty text description for inference
            if not text_description:
                raise ValueError(
                    f"Image at index {index} (path: {rgb_rel_path}) has no text description. "
                    f"Every image must have a non-empty text description for inference. "
                    f"Normalized path used for lookup: {normalized_rgb_path}"
                )
            if not isinstance(text_description, str):
                raise ValueError(
                    f"Image at index {index} (path: {rgb_rel_path}) has non-string text description "
                    f"(type: {type(text_description)}). Every image must have a string text description."
                )
            if len(text_description.strip()) == 0:
                raise ValueError(
                    f"Image at index {index} (path: {rgb_rel_path}) has empty text description. "
                    f"Every image must have a non-empty text description for inference."
                )
        
        # Always include text in output (empty string if not found and not required)
        other["text"] = text_description
