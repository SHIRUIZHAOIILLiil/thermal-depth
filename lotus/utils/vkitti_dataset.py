...
class VKITTIDataset(Dataset):
    #mod add text_descriptions_path
    def __init__(self, root_dir, transform=None, norm_type='truncnorm', truncnorm_min=0.02, text_descriptions_path=None):
        """
        Args:
            ...
            text_descriptions_path (str, optional): Path to JSON file containing text descriptions.
        """
        ...
        # mod Load text descriptions if provided
        self.text_descriptions = {}
        if text_descriptions_path and os.path.exists(text_descriptions_path):
            with open(text_descriptions_path, 'r') as f:
                self.text_descriptions = json.load(f)
            print(f"Loaded {len(self.text_descriptions)} text descriptions from {text_descriptions_path}")
            
            # Check text description lengths
            # desc_lengths = []
            # for path, desc_data in self.text_descriptions.items():
            #     if isinstance(desc_data, dict) and "description" in desc_data:
            #         desc_lengths.append(len(desc_data["description"]))
            #     elif isinstance(desc_data, str):
            #         desc_lengths.append(len(desc_data))
            
            # if desc_lengths:
            #     print(f"VKITTI Text description statistics:")
            #     print(f"  Min length: {min(desc_lengths)} chars")
            #     print(f"  Max length: {max(desc_lengths)} chars")
            #     print(f"  Average length: {sum(desc_lengths)/len(desc_lengths):.1f} chars")
            #     print(f"  Descriptions > 500 chars: {sum(1 for l in desc_lengths if l > 500)}")
        else:
            print(f"No VKITTI text descriptions file found at {text_descriptions_path}")
    ...
    def __getitem__(self, idx):
        ...
        # mod Add text description
        text_desc_data = self.text_descriptions.get(image_path, {})
        if isinstance(text_desc_data, dict) and "description" in text_desc_data:
            text_desc = text_desc_data["description"]
        elif isinstance(text_desc_data, str):
            text_desc = text_desc_data
        else:
            text_desc = ""
        example['text_description'] = text_desc
